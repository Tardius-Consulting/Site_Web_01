...
## Contribuição
Para enviar seu código para o repositório você deve primeiro usar o comando "git pull origin main" para verificar se não existe nenhuma atualização, depois fazer o commit das modificações que você realizou em uma banch diferente da main, você pode verificar a branch que você está com o comando 'git branch' e pode criar uma nova branch com o comando 'git cheackout -b novaBranch', estando em uma nova branch você pode fazer o commit com o comando 'git commit -m "<tipo>(<escopo>): <descrição curta>"', após o commit você usar o comando 'git push -u origin novaBranch' para enviar as atualizações para o repositório.
